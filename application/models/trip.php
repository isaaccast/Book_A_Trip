...
class Trip extends CI_Model {
	...
	public function users_trips()
	{
		...
		return $this->db->query("SELECT des.place, DATE(des.start) as start, DATE(des.end) as end, des.plan, trips.user_id, trips.des_id FROM des LEFT JOIN trips ON des.id = trips.des_id WHERE trips.user_id = $user")->result_array();
	}

	public function others()
	{
		$user = $this->session->userdata['id'];
		$query = "SELECT des.place, DATE(des.start) as start, DATE(des.end) as end, des.plan, des.planned_by, des.id FROM des WHERE des.id NOT IN(SELECT trips.des_id FROM trips WHERE trips.user_id = $user) GROUP BY des.place";
	...
	{
		$query = "SELECT des.place, des.plan, des.start, des.end, des.planned_by, users.name FROM des LEFT JOIN trips ON des.id = trips.des_id LEFT JOIN users ON trips.user_id = users.id WHERE des.id = $trip_id";
	...
	{
		$user = $this->session->userdata['id'];
		$query = "SELECT users.name FROM users LEFT JOIN trips ON users.id = trips.user_id LEFT JOIN des ON trips.des_id = des.id WHERE des.id = $des_id AND des.planned_by != users.name";
		if($this->db->query($query)){
			return $this->db->query($query)->result_array();
		}else{
